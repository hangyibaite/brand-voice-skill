...
Read `intake.md`. Run the protocol once per chosen category, in order. Do not skip this. The live-writes outweigh the past samples.

## Step 4: Analyze

Read `analyze.md`. Run extraction against samples + live-writes. For each chosen category, fill the schema in `references/[category].md` using only what the samples and live-writes show.
...
Read `assets/output-template.md`. Build the personalized skill at `/mnt/user-data/outputs/[name]-voice/` mirroring this skill's structure exactly:
...
Copy `assets/anti-ai.md` and `references/iterate.md` verbatim into the new skill.
